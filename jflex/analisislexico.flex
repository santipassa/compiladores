...
%{
	...
	public AnalisisLexico(java.io.FileInputStream r, ComplexSymbolFactory sf){
		this(r);
		this.sf=sf;
	}
	public Symbol symbol(String plaintext,int code){
	    return sf.newSymbol(plaintext,code,new Location("",yyline+1, yycolumn +1), new Location("",yyline+1,yycolumn+yylength()));
	}
	public Symbol symbol(String plaintext,int code,Integer number){
	    return sf.newSymbol(plaintext,code,new Location("",yyline+1, yycolumn +1), new Location("",yyline+1,yycolumn+yylength()),number);
	}
%}
%eofval{
