...
public class CheckTypeVisitor implements ASTVisitor<Type> {
	...
	public Type visit(Statement_ifelse expr) {
		if ((expr.getExpr1().getType().toString()!="boolean")||(expr.getExpr2().getType().toString()!="boolean"))
			System.out.println("ERROR");
		expr.getStatement().accept();
		return null;
	}
	...
	public Type visit(Bin_op expr) {
		if (!(expr.getExpr1().getType().equals(expr.getExpr2().getType()) && typeOk(expr.getExpr1().getType())))
			System.out.println("ERROR");
		return null;
